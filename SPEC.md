...
hash-forge run --seed 123 --generations 100 --starter-cap 8 --starter-cap-after 10 --no-champions
...
`--starter-cap <n>`, `--starter-cap-after <generations>`, and
`--no-starter-cap` control starter-lineage survivor pressure. The cap changes
which candidates are allowed to breed from survivor slots after the warmup, but
does not alter raw quick or deep scores.
...
starter-cap, no-novelty, no-crossover, no-starter, no-refresh, bare, and
refresh-strong settings across a generated seed range or explicit comma-separated
seed list. It should disable champion starters for fairness, print aggregate
results, then write `out/policy.md` and `out/policy.csv`.
...
the same scoring suite used for evolved candidates. It should print a compact
score table and write `out/baselines.md`. Baseline interpretation should compare
fail flags first, then deep score, then speed and exported instruction count.
These baselines are lab reference marks, not cryptographic proof.

`champions` lists saved custom hashes, ranks them by candidate quality, rescoring
records first when their stored scoring fingerprint differs from the current
established-baseline fingerprint. It should write `out/champions.md`.
...
- Champion records are loaded before evolution and saved after completion, never
  inside the hot scoring loop.
...
best-candidate operator histogram and baseline comparison scores, including
...
random immigrants, novelty-lane activity, starter-cap displacement telemetry,
plus the number of champion starters loaded and the best candidate source
ancestry. It should include an improvement timeline showing
...
`out/baselines.md` should be a human-readable established-hash baseline report
generated by the baselines command.
`out/champions/*.hfch` should store plain-text champion records with serialized
VM instructions, scores, audit summary, source run, and scoring fingerprint.
`out/champions.md` should be a human-readable champion leaderboard generated by
the champions command.
...
`out/baselines.md`, verify champion save/list/load and fingerprint rescore
behavior, and compile `out/best.c` as standalone C.
